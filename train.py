...
FILE = Path(__file__).resolve()   #获取该文件地址
ROOT = FILE.parents[0]
...
LOCAL_RANK = int(os.getenv('LOCAL_RANK', -1))   #进程内，GPU 编号，非显式参数，由 torch.distributed.launch 内部指定。比方说， rank = 3，local_rank = 0 表示第 3 个进程内的第 1 块 GPU。
RANK = int(os.getenv('RANK', -1))    #表示进程序号，用于进程间通讯，表征进程优先级。rank = 0 的主机为 master 节点。
WORLD_SIZE = int(os.getenv('WORLD_SIZE', 1))  #全局进程个数。
...
def train(hyp,
    ...
    # Model parameters
    hyp['box'] *= 3. / nl  # scale to layers   nl:模型有多少层
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                pred = model(imgs.to(torch.float32))  # forward
            ...
            if RANK in [-1, 0]:
                ...
                callbacks.run('on_train_batch_end', ni, model, imgs.to(torch.float32), targets, paths, plots, opt.sync_bn)
            # end batch ------------------------------------------------------------------------------------------------
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_wandb_resume(opt) and not opt.evolve:  # resume an interrupted run
        ...
        opt.project = str(opt.project)  # checks
